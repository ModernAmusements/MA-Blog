...
```mermaid
flowchart TD
    A[Screenshot Image] --> B[Kartentyp-Erkennung]
    B --> C{Typ?}
    C -->|Pokemon| D[Zonen-Extraktion]
    C -->|Trainer| E[Trainer-Verarbeitung]
    C -->|Energy| F[Energy-Verarbeitung]
    
    D --> G[Name Zone OCR]
    D --> H[KP Zone OCR]
    D --> I[Angriff Zone OCR]
    D --> J[Schwäche/Rückzug Zone OCR]
    
    G --> K[Signal-Korrektur]
    H --> K
    I --> K
    J --> K
    
    K --> L[Karten-Abgleich-Engine]
    
    L --> M{Konfidenz >= 60%?}
    M -->|Ja| N[collection.db]
    M -->|Nein| O[failed_to_capture/]
...
flowchart LR
    ...
    B -->|Kombinieren| C[pokewiki_scraped_all.json]
    C -->|Index| D[cards.db SQLite]
    
    E[Fähigkeiten Scraping] -->|Scrape| F[abilities.json]
    F --> C
```
...
```mermaid
flowchart TD
    A[OCR-Signale] --> B{Exakter Name + Set?}
    B -->|Ja| C[95% Konfidenz]
    B -->|Nein| D{Name + KP?}
...
flowchart TD
    ...
    C --> E{Nach Set}
    C --> F{Nach Seltenheit}
    C --> G{Fehlgeschlagene Erfassungen}
    
    D --> H[Statistiken anzeigen]
    E --> H
    F --> H
    G --> H
```
...
```mermaid
flowchart TD
    A[pokewiki.de Set-Seite] --> B[scrape_pokewiki.py]
    B --> C[Kartenlinks extrahieren]
    C --> D[Für jede Karte]
    D --> E[Kartenseite abrufen]
    E --> F[HTML parsen]
    F --> G[Daten extrahieren]
    G --> H[pokewiki SET json]
    
    I[pokewiki.de Karten-Seite] --> J[scrape_abilities.py]
    J --> K[Karten mit Fähigkeit finden]
    K --> L[Fähigkeit extrahieren]
    L --> M[abilities.json]
    
    H --> N[Alle Sets kombinieren]
    M --> N
