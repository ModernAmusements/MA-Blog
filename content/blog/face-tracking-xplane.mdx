---
title: LiDARSight
description: How we built an open-source 6DoF head tracking system using nothing more than an iPhone
date: 2026-04-13
...
```mermaid
flowchart LR
    A[Infrared Dot Projector] --> B[Face Surface]
    B --> C[IR Camera Array]
    C --> D[Depth Map Computation]
    D --> E[Face Mesh Tracking]
```
...
```mermaid
flowchart LR
    A[LiDAR Emitter] --> B[Environment Surface]
    B --> C[LiDAR Sensor]
    C --> D[Point Cloud Reconstruction]
    D --> E[3D Mesh Generation]
```
...
![LiDARSight Architecture](/images/projects/LiDARSight/LiDARSight_Architecture.png)
...
flowchart TB
    ...
    C --> D[Extract simd_float4x4 transform]
    D --> E[Convert to Euler Angles and Position]
    E --> F{Protocol Mode}
...
![Protocols Comparison](/images/projects/LiDARSight/protocols.png)
...
flowchart LR
    subgraph Debugger["macOS Debugger"]
        A[TCP Listener :4242] --> B[Length Prefix Parser]
        B --> C[Packet Parser]
        C --> D[One Euro Filter]
        D --> E[Deadzone Filter]
        E --> F[Curve Mapping]
        ...
        H[3D Windshield SceneKit] --> F
        I[Values Panel RAW FILTERED OUTPUT] --> F
    end
```
...
```mermaid
flowchart TB
    input[New Sample] --> A[Calculate Velocity]
